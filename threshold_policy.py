...
class THR:
    ...
    def solve(self, env):
        ...
        self.thr = env.r
        self.true_state_thr = env.true_state_thr
        for j in range(mc):
            print(j)
            all_rewards = []
            all_drops = []
    ...
    def get_action(self, env):    # action for every agent
        ...
        if self.true_state_thr:
            state = env.state_each_agent[:,0]
        else:
            state = env.expected_belief_x_t_each_agent
        agents_sending = np.where(state > self.thr)[0]
        action0[agents_sending] = 1
