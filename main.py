import rllib_nagent_action_ppo_ps, rllib_mfagent_action_ppo, rllib_saagent_action_ppo, random_policy, always_send,\
    threshold_policy
import time
import utils
...
def run(args):
    assert(args.config in ('na_dec', 'na_cen', 'rnd', 'a1', 'pomfc', 'thr'))
    assert(args.state in ('t','b'))
    assert(args.rnn in ('rnn','no'))
    assert(args.ch in ('known','unknown'))
    assert(args.particles in ('use','dont'))
    assert(args.cont_state in ('cont','disc'))
    assert(args.weighted_action in ('w','u'))
    ...
    results_dir = utils.create_file_path(number_of_agents, config, use_true_state, use_belief_state, rnn, ch,
                                         use_particles, args.cont_state, args.weighted_action, drops,
                                         dt=dt, r=r, true_state_thr=true_state_thr)

    env = AOI_env(simulation_timesteps, number_particles, number_of_agents, config, results_dir, action_space_size,
                  weighted_action, use_true_state, use_belief_state, lambda_val, channel_known, use_particles, eval,
    ...
    if config in ('pomfc'):
        ...
        solver = random_policy.RND(env_creator, rnn, results_dir=results_dir)
    elif config == 'thr':
        solver = threshold_policy.THR(env_creator, rnn, results_dir=results_dir)
    elif config == 'a1':
        solver = always_send.A1(env_creator, rnn, results_dir=results_dir)
    else:
        raise NotImplementedError
    begin = time.time()
